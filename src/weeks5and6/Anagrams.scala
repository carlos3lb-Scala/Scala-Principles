...
object Anagrams {
   ...
   */
  val dictionary: List[Word] = List("ate", "eat", "cac", "ateCarlosLopezBartolome", "caozrapeberelltaolomtos")
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(x => x).map { case (key, value) => (key, value.length) }.toList.sortBy(_._1)
  ...
  def wordAnagrams(word: Word): List[Word] = (dictionaryByOccurrences find (x => x._1 equals (wordOccurrences(word))) get)._2
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagramsInner(o: Occurrences): List[Sentence] = o match {
      case List() => List(List())
      case xs => for(
          i <- combinations(xs) if dictionaryByOccurrences.keySet(i);
          j <- dictionaryByOccurrences(i);
          s <- sentenceAnagramsInner(subtract(xs, i))
        ) yield { j :: s }
    }
  ...
  def main(args: Array[String]) {
    val word = "aaccba"
    println(wordOccurrences(word))
    println(dictionary)
    val dictionaryByOccurrences = dictionary.groupBy(x => wordOccurrences(x))
